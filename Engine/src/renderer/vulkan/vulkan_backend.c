...
// and make sure they exist. Validation layers should only be enabled on non-release builds.
    ...
    for(u8 i = 0; i < context.swapchain.max_frames_in_flight; ++i)
    {
        ...
        // Create the fence in signaled state, indicating that the first frame has already been "rendered".
        // This will prevent the application from waiting indefinitely for the first frame to rende since it
        // cannot be rendered until a frame is "rendered" before it.
        vulkan_fence_create(&context, TRUE, &context.in_flight_fences[i]);
    }

    // In filght fences should not yet exist at this point, so clear the list. These are stored in pointers
...
void vulkan_renderer_backend_on_resized(renderer_backend* backend, u16 width, u16 height) 
{
    ...
    KINFO("Vulkan renderer backend->resized: w/h/gen: %i/%i/%1lu", width, height, context.framebuffer_size_generation);
}
...
b8 vulkan_renderer_backend_begin_frame(renderer_backend* backend, f32 delta_time) {
    ...
    if(context.recreating_swapchain)
    {
        ...
        KINFO("Recreatig swapchain, booting.");
        return FALSE;
    }
...
b8 vulkan_renderer_backend_end_frame(renderer_backend* backend, f32 delta_time) {
    ...
    // Each semaphore waits on the corresponding pipeline stage stage to complete. 1:1 ratio.
...
void regenerate_framebuffers(renderer_backend* backend, vulkan_swapchain* swapchain, vulkan_renderpass* renderpass)
{
    ...
    {
        // TODO: make this dynamic based on the currentlt configures attachments
...
b8 recreate_swapchain(renderer_backend* backend)
{
    ...
    // Mark as recreating if the dimenison are valid.
    ...
    // Sync the framebuffer size width the cached sizes.
