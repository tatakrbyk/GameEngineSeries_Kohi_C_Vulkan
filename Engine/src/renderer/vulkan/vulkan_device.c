...
b8 vulkan_device_create(vulkan_context* context)
{
    ...
    // cmd-> vkinfo -> VkQueueFamilyProperties:
...
)
{
    ...
    // Print out some info about the deice
    KINFO("       %d |       %d |       %d |        %d | %s",
        out_queue_info->graphics_family_index != -1,
        out_queue_info->present_family_index != -1,
        out_queue_info->compute_family_index != -1,
        ...
        (!requirements->transfer || (requirements->transfer && out_queue_info->transfer_family_index != -1)))
    {
        ...
        KTRACE("Presents Family Index: %i", out_queue_info->present_family_index);
        ...
        if(requirements->device_extension_names)
        {
            ...
            if(available_extension_count != 0)
            {
                ...
                for(u32 i = 0; i < required_extension_count; ++i)
                {
                    ...
                    if(!found)
                    {
                        KINFO("Required extension not fount: '%s', skipping device.", requirements->device_extension_names[i]);
