...
b8 vulkan_object_shader_create(vulkan_context* context,  vulkan_object_shader* out_shader)
{
    ...
    for (u32 i = 0; i < OBJECT_SHADER_STAGE_COUNT; i++)
    {
        ...
        stage_types[i], i, out_shader->stages)){
            KERROR("Unable to create %s shader module for '%s'.",stage_type_strs[i], 
    ...
    {
        attribute_descriptions[i].binding = 0; // binding index - shoult match binding desc
...
void vulkan_object_shaader_use(vulkan_context* context, struct vulkan_object_shader* shader){
